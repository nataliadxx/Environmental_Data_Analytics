...
------------ | ---------------------------------------------------------------------------------------
NTL-LTER     | Studies on several lakes in the North Temperate Lakes District in Wisconsin, USA
USGS Site    | Streamflow data from the USGS streamflow gage site 02085000
EPA Air      | Data from air quality monitoring of PM2.5 and ozone in North Carolina in 2017 and 2018
...
```{r, include=FALSE}
ntl.nutrient <- read.csv("../Data/Raw/NTL-LTER_Lake_Nutrients_Raw.csv")
head(ntl.nutrient)
ntl.nutrient$sampledate <- as.Date(ntl.nutrient$sampledate,"%m/%d/%y")
```
...
```{r,include=FALSE}
ntl.tn.summaries <- ntl.processed %>%
  filter(!is.na(tn_ug)) %>%
  group_by(lakename) %>%
  summarise(meanTN = mean(tn_ug),
            ...
            maxTN = max(tn_ug),
            sdTN = sd(tn_ug))
ntl.tp.summaries <- ntl.processed %>%
  filter(!is.na(tp_ug)) %>%
  group_by(lakename) %>%
...
```{r,echo=FALSE}
kable(ntl.tn.summaries,caption = "Summary for total N")
kable(ntl.tp.summaries,caption = "Summary for total P")
```
...
```{r,echo=FALSE,fig.cap="Total N over time for each lake"}
ggplot(ntl.processed,aes(x=sampledate,y=tn_ug,color=lakename)) +
  geom_line() 
```
```{r,echo=FALSE,fig.cap="Total P over time for each lake"}
ggplot(ntl.processed,aes(x=sampledate,y=tp_ug,color=lakename)) +
  geom_line() 
```
